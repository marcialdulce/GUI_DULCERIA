...
class PanelInicio(tk.Frame):
    def __init__(self, parent, controlador, comando_navegar):
        ...
        def crear_tarjeta(titulo):
            ...
            tarjeta = tk.Frame(tarjeta_externa, bg=BLANCO, bd=1, relief="solid")
        ...
        tk.Label(tarjeta_catalogo, text=str(len(productos_disponibles)), bg=ROSA_MENU, fg=BLANCO, font=("Arial", 14, "bold"), width=3, height=1).pack(pady=(25, 8))
        tk.Label(tarjeta_catalogo, text="unidades en venta", bg=BLANCO, fg="#555555", font=("Arial", 11)).pack()
        tk.Label(tarjeta_catalogo, text="🛒", bg=BLANCO, fg="#555555", font=("Segoe UI Emoji", 38)).pack(pady=20)

        # TARJETA 2
        tarjeta_stock = crear_tarjeta("Stock más bajo")
        tk.Label(tarjeta_stock, text="ARTÍCULO                 STOCK", bg=BLANCO, fg="#555555", font=("Arial", 8, "bold")).pack(anchor="w", padx=18, pady=(20, 8))

        for producto in productos_bajo_stock:
            fila = tk.Frame(tarjeta_stock, bg=BLANCO)
            fila.pack(fill="x", padx=18, pady=2)
            tk.Label(fila, text=producto["nombre"], bg=BLANCO, fg="#555555", font=("Arial", 9), anchor="w").pack(side="left")
            tk.Label(fila, text=str(producto["stock"]), bg=BLANCO, fg="#555555", font=("Arial", 9), anchor="e").pack(side="right")

        tk.Label(tarjeta_stock, text="📦", bg=BLANCO, fg="#555555", font=("Segoe UI Emoji", 38)).pack(pady=20)

        # TARJETA 3
        tarjeta_cobro = crear_tarjeta("Cobro de artículos")
        tk.Label(tarjeta_cobro, text="¡Caja abierta y lista\npara operar!", bg=BLANCO, fg="#555555", font=("Arial", 11), justify="center").pack(pady=(25, 15))
        tk.Button(tarjeta_cobro, text="INICIAR VENTA", bg="#F4D03F", fg=NEGRO, activebackground="#E5C12E", font=("Arial", 10, "bold"), relief="flat", cursor="hand2", padx=15, pady=8, command=lambda: self.comando_navegar("NUEVA VENTA")).pack()
        tk.Label(tarjeta_cobro, text="🧾", bg=BLANCO, fg="#555555", font=("Segoe UI Emoji", 38)).pack(pady=18)